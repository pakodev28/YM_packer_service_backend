...
from .serializers import (
    CreateOrderSerializer, GetTableSerializer, GetTokenSerializer,
    SelectTableSerializer, SelectPrinterSerializer, SignUpSerializer, ReadOrderSerializer)
...
def select_table(request, id):
    serializer = SelectTableSerializer(data=request.data, context={'request': request})
    serializer.is_valid(raise_exception=True)
...
def select_printer(request):
    serializer = SelectPrinterSerializer(data=request.data, context={'request': request})
    serializer.is_valid(raise_exception=True)
    serializer.save()
    return Response(serializer.data, status=status.HTTP_201_CREATED)
